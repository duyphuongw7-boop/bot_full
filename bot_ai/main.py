...
bot_state = {
    ...
    "is_running": False,
    "kill_switch": False,
    "latest_indicators": {}  # Đã thêm để cache dữ liệu cho API Dashboard
}
...
def execute_trade(side, price, reason, ind_data, is_closing=False, amt_to_close=0):
    try:
        if is_closing:
            ...
            if not account_info: 
                custom_log("⚠️ Không lấy được số dư tài khoản. Bỏ qua lệnh.")
                return
                
            available_usdt = float(account_info['availableBalance'])
            trade_margin = available_usdt * 0.95 
            if trade_margin < 5:
                custom_log("Số dư quá thấp để mở lệnh!")
                return
        ...
        if not order or 'orderId' not in order:
            custom_log("⚠️ Không tạo được lệnh (API lỗi hoặc bị từ chối).")
            return

        is_filled = check_order_filled(order['orderId'])
        if not is_filled:
            custom_log("⚠️ Lệnh không fill hoàn toàn, xin kiểm tra lại tài khoản!")

        trades = safe_api_call(client.futures_account_trades, symbol=SYMBOL, limit=20)
        if not trades: return
        
        matched_trades = [t for t in trades if t['orderId'] == order['orderId']]
        fee = sum(float(t['commission']) for t in matched_trades)
        
        if is_closing:
            pnl = sum(float(t['realizedPnl']) for t in matched_trades)
            with state_lock:
                ...
                # FIX BUGS RACE CONDITION: Ép đóng vị thế ngay lập tức
                bot_state["current_position"] = "NONE"
            ...
            with state_lock:
                bot_state["entry_fee"] = fee
            custom_log(f"✅ ĐÃ MỞ {side} | KL: {qty} | Phí: {fee:.4f} USDT")
...
def run_bot():
    ...
    while True:
        try:
            ...
            # FIX BUGS WEB CRASH: Chuyển đổi format indicator để cache cho Frontend Dashboard
            formatted_inds = {}
            ...
            if current_pos != "NONE":
                ...
                c_buy_1h = inds['1h']['macd_prev'] > inds['1h']['macd_prev2']
                ...
                c_sell_1h = inds['1h']['macd_prev'] < inds['1h']['macd_prev2']
...
@app.get("/api/stats")
async def api_stats():
    # Đã gỡ bỏ tính toán MACD/StochRSI ở luồng API để tránh làm nghẽn ứng dụng
    with state_lock:
...
async def toggle_kill_switch(req: KillSwitchRequest):
    ...
    if status and amt != 0:
        try:
            ...
            # FIX LỖI: Update ngay state khi Kill Switch đóng lệnh
            with state_lock:
                bot_state["current_position"] = "NONE"
                bot_state["position_amt"] = 0.0
